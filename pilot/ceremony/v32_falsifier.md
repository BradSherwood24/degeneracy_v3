...
n with `n + fee(n) <= 2.00 - E - W`, continuously re-solve/replace it (amend-first, cancel -> confirm ->
create as the fallback; never two live rests), and on fill immediately TAKE both pin wings as a taker; the $2 pin pays $2.00 at
...
- 2026-09-15 ~18:10Z -- MECHANICS CLARIFICATION (amend-first replace; Brad, verbatim: "Yea, I agree. Use the cancel and recreate flow as a backup if our post to ammend the order fails. Go ahead and build that"). The mid-window replace is now AMEND-FIRST: a same-bucket requote AMENDS the resting order in place (Kalshi Amend Order V2, `POST /portfolio/events/orders/{id}/amend`, same order_id; a price change forfeits queue position EXACTLY as cancel+create did), and cancel -> confirm -> create is the FALLBACK on any non-2xx/timeout amend. Bucket changes (a different ticker; an amend cannot change the ticker) and the quote-end cancel are UNCHANGED. "Never two live rests" is preserved (the one order is amended, not stacked). Context: the old SEQUENTIAL cancel -> confirm -> create left NOTHING resting for ~0.7 s per replace (60-90 s per busy hour at ~77 replaces/h) -- the cause of the 2026-09-15 14:00Z window's missed fill (the rest was cancelled at 13:45:20Z and a qualifying print landed in the no-rest gap). This is a MECHANICS CLARIFICATION, NOT a threshold/param/sha change: no [pin], the `n >= 30` count, the params sha `0ac697957c69a004e45d49505cce1084aaeb2e50bbaea45fe60bfbe0911c80dc`, or the STATUS line is touched. An amend IS a replace for `replace_count`, the A_REPLACE (replace_rate) alarm, and the ledger `replaces` counter; the amend keeps the order resting continuously and reaches the new price one RTT sooner, so its lock is >= the cancel+create lock (the golden test `test_core_live_fill_matches_lagging_reference` still reproduces the +10.36c reference exactly -- the reference model is itself single-lag). Registered at n=2 live sets (well before the n>=30 verdict), so the registered-specs rule is honoured. IN-PLACE EDIT above: the "What is being judged" line's `(cancel -> confirm -> create, never two live rests)` now reads `(amend-first, cancel -> confirm -> create as the fallback; never two live rests)`. Mirrored in code: `service.v32.actions.ActionKind.AMEND_REST`/`WOULD_AMEND_REST` (+ `updated_client_order_id`), `service.v32.events.OrderAmended`, `service.v32.core` (`_emit_amend`, `_apply_amended`, `amend_in_flight`, the requote gate), `service.v32.executor` (`_amend_rest`/`_amend_body`/`amend_path`/`_amend_fallback`), journaled (`amend_rest`/`amend_confirmed`/`amend_failed`/`amend_fill`) + counted (`amends_attempted`/`amends_confirmed`/`amends_failed`/`amend_fallbacks`/`fills_on_amend`) by `run_v32` + `service.v32.ledger`, surfaced by `service.v32.report`.
...
- 2026-09-19 ~22:40Z -- MEASUREMENT CLARIFICATION 3 (fill-rate gate -> capture ratio; Brad, verbatim: "I agree with the second there. Looks like our dry spell has ended"). Context: live is at n=7 with mean lock +11.1c, 100% positive, exec gap -0.6c, one-legged 0, but the fill rate is ~1.5 sets/day because the September tape carries ~1/3 the pump traffic of June-July (train corpus fills/ISO-week at the stale E=15 gate were 41,34,32,32,35,28 in W26-W31 vs 4,13 in W35-W36; the 2.0/day [pin] was calibrated on the W36 forward week, itself lean). The `>= 2.0 sets/day` gate therefore measures the TAPE REGIME (the weather), not our EXECUTION. Claude's proposal, verbatim: "Register now, at n=6, that fill rate is judged against pump availability, for example live fills divided by ideal-shadow fills at our threshold, which is currently about 55% and tests execution rather than the weather." Brad agreed ("the second there" = this proposal) on 2026-09-19 ~22:40Z.
  - DEFINITION (bounded per-WINDOW fraction; N1 fix from the PR #66 review): the CAPTURE RATIO = (number of armed+bucket windows with BOTH a completed live set AND an ideal-shadow E=0.10 fill) / (armed+bucket windows with an ideal-shadow E=0.10 fill inside the quoting window). BOTH numerator and denominator count each window 0/1 over ARMED windows carrying a spot bucket (ledger rows with `effective_mode` = `armed` AND a `spot_bucket_ticker`), from re-arm 2026-09-14 23:00Z onward (the SAME evaluation window as the other gates). A "live completed set" is per the 2026-09-18 clarification (one rest-fill event with BOTH wings filled); a window with TWO completed sets (contracts>=2 partial fills) still counts as ONE captured window, and a live-set-without-shadow window never pushes the ratio above 1 (the ratio is bounded to [0, 1] -- you cannot capture more pumps than the shadow proved available). The shadow fills at most once per window by construction (the T-15..T-5 window gate, MEASUREMENT CLARIFICATION 2 / PR #54). A window the live path STOOD DOWN for ANY pilot-side reason (replace_rate alarm, stale wing, phantom, cancel_failed) has no completed set and so counts as a MISS -- that is execution/design, and is exactly what this gate must catch. A window the pilot never ran (box outage / reboot / task not started) leaves no armed row and does not count either way.
  - N_MIN GATE (found on the live ledger 2026-09-19 23:00Z, in this same registration): the ideal shadow honours `params.n_min` (= 0.05, a pinned value in the frozen params sha). The live path stands down `n_below_min` when the continuously-solved n < `n_min` and would NEVER rest at such an n, so a shadow fill at a solved n below `n_min` is NOT a live-reachable counterfactual -- exactly the class MEASUREMENT CLARIFICATION 2 / PR #54 excluded for the quoting-window gate ("a print the live path could never have taken"). Such a fill is excluded from the capture-ratio DENOMINATOR and from the shadow mean-lock / execution-gap stats. Evidence: the 2026-09-19 23:00Z window (bucket B81250) -- the shadow filled at offer 0.96 (n = 0.04) on a 16-lot print at 0.97 while the live path stood down `n_below_min` the whole hour. Mirrored in code going forward (`core._shadow_on_trade` does not fill below `n_min`, journals `shadow_fill_below_min`, counted as `shadow_fills_below_min`) AND for EXISTING ledger rows (the report derives the shadow n as `1 - offer` -- the record's top-level `n` is the end-of-window resolved n, usually None -- and excludes derived-below-min fills), so history is measured the same way. This is a MEASUREMENT DEFINITION, not a threshold change: no `[pin]`, the `n >= 30` count, the params sha, or the STATUS line is touched.
  - PIN: `V32_CAPTURE_RATIO_MIN` = 0.50 (Claude's proposed number; number to be confirmed by Brad at merge). At n >= 30 the verdict now gates on `capture ratio >= 0.50` IN PLACE OF `fill rate >= 2.0 sets/day`. The fill-rate [pin] (`V32_FALSIFIER_MIN_FILL_RATE_PER_DAY` = 2.0) stays DEFINED in code (add-only law) and the report keeps PRINTING sets/day, now labelled "(info, superseded as a gate by Registration 3)". No other [pin], the `n >= 30` count, the params sha `0ac697957c69a004e45d49505cce1084aaeb2e50bbaea45fe60bfbe0911c80dc`, or the STATUS line is touched.
  - RATIONALE: the 2.0/day pin measured availability (the weather), not execution; the ideal shadow already MEASURES availability, so live/shadow at our threshold isolates execution. Registered at n=7 (well before the n >= 30 verdict), so the registered-specs rule -- no re-spec on the same evaluation window -- is honoured: the gate is pinned before the window it will judge closes. This is a MEASUREMENT DEFINITION, not an economics change; the mean-lock, %positive, exec-gap and one-legged gates are untouched.
  - CURRENT VALUE (read-only copy of the live ledger, `pilot/ledger/v32_ledger.jsonl`, fresh 125-row snapshot 2026-09-19): the shadow E=0.10 recorded 13 armed+bucket fills, of which ONE (the 09-19 23:00Z below-min fill above, offer 0.96 / n 0.04) is excluded by the n_min gate -> 12 valid shadow-available windows; the live path captured 7 -> capture ratio 7/12 = 58.3% (>= 0.50). The 5 misses among the 12: 1 pre-PR#54 counting artifact (09-15 04Z), 2 rest-absent-at-print during replace churn (09-15 14Z/15Z), 1 phantom-resting stand-down (09-15 18Z, fixed by PR #56), 1 replace-rate alarm stand-down (09-18 14Z). (Before the N1/n_min fixes the same fresh ledger read 7/13 = 53.8%; the n_min exclusion of the 23:00Z window restores 7/12 = 58.3%.)
