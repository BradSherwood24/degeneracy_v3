...
- 2026-09-19 ~22:40Z -- MEASUREMENT CLARIFICATION 3 (fill-rate gate -> capture ratio; Brad, verbatim: "I agree with the second there. Looks like our dry spell has ended"). Context: live is at n=7 with mean lock +11.1c, 100% positive, exec gap -0.6c, one-legged 0, but the fill rate is ~1.5 sets/day because the September tape carries ~1/3 the pump traffic of June-July (train corpus fills/ISO-week at the stale E=15 gate were 41,34,32,32,35,28 in W26-W31 vs 4,13 in W35-W36; the 2.0/day [pin] was calibrated on the W36 forward week, itself lean). The `>= 2.0 sets/day` gate therefore measures the TAPE REGIME (the weather), not our EXECUTION. Claude's proposal, verbatim: "Register now, at n=6, that fill rate is judged against pump availability, for example live fills divided by ideal-shadow fills at our threshold, which is currently about 55% and tests execution rather than the weather." Brad agreed ("the second there" = this proposal) on 2026-09-19 ~22:40Z.
  - DEFINITION: the CAPTURE RATIO = (live completed sets) / (ideal-shadow E=0.10 fills inside the quoting window), both counted over ARMED windows carrying a spot bucket (ledger rows with `effective_mode` = `armed` AND a `spot_bucket_ticker`), from re-arm 2026-09-14 23:00Z onward (the SAME evaluation window as the other gates). A "live completed set" is per the 2026-09-18 clarification (one rest-fill event with BOTH wings filled). The shadow fills at most once per window by construction (the T-15..T-5 window gate, MEASUREMENT CLARIFICATION 2 / PR #54), so the denominator counts each qualifying window at most once. A window the live path STOOD DOWN for ANY pilot-side reason (replace_rate alarm, stale wing, phantom, cancel_failed) yields 0 completed sets and so counts as a MISS -- that is execution/design, and is exactly what this gate must catch. A window the pilot never ran (box outage / reboot / task not started) leaves no armed row and does not count either way.
  ...
  - CURRENT VALUE (read-only copy of the live ledger, `pilot/ledger/v32_ledger.jsonl`, snapshot 2026-09-19): shadow E=0.10 filled 12 armed+bucket windows; the live path captured 7 -> capture ratio 7/12 = 58.3% (>= 0.50). The 5 misses: 1 pre-PR#54 counting artifact (09-15 04Z), 2 rest-absent-at-print during replace churn (09-15 14Z/15Z), 1 phantom-resting stand-down (09-15 18Z, fixed by PR #56), 1 replace-rate alarm stand-down (09-18 14Z).
  ...
  - Mirrored in code: `service.v32.falsifier_pins.V32_CAPTURE_RATIO_MIN` (new [pin]); `service.v32.report.build_falsifier_scoreboard` computes `capture_live_sets`/`capture_shadow_fills`/`capture_ratio` and the verdict gates on the ratio (fill rate stays computed + printed as info); the FALSIFIER SCOREBOARD prints `capture ratio = live X / shadow Y = Z%` with the [pin]; asserted in `tests/test_v32_falsifier_pins.py` (add-only) and `tests/test_v32_report_scoreboard.py`.
